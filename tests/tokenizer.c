...
int main(int argc, char** argv){
    ...
    if(!read_file(&stream, argv[1], "r")){
        fprintf(stderr, "[ERROR] Could Not Open/Read File '%s'\n", argv[1]);
        return 1;
    }

    Tokenizer tokenizer;
    tokenizer.data = stream.data;
    tokenizer.line = 0;
    tokenizer.column = 0;
    tokenizer.pos = 0;

    for(Token token = get_next_token(&tokenizer); token.value.as_str != NULL; token = get_next_token(&tokenizer)){
        printf("line: %i, column: %i '%.*s'\n", token.line, token.column, token.size, token.value.as_str);
    }
    printf("\n");
