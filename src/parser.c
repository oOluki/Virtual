#ifndef VIRTUAL_PARSER_C
#define VIRTUAL_PARSER_C

#include "lexer.h"
#include "parser.h"
#include "parse_instRW.h"
...
uint8_t get_inst_op(const Token inst_token){
    if(inst_token.type != TKN_RAW)
        return INST_ERROR;
    
    return _get_inst_op(inst_token.value.as_str, inst_token.size);
}
...
int pre_parse_inst_operand(Parser* parser, Token* _token, uint64_t absolute_program_position){
    ...
    if(token.type == TKN_LABEL_REF){
        ...
        token = resolve_token(&parser->labeler, token);
        if(token.type == TKN_ERROR){
            if(token.size > 1 && (parser->labeler.flags & LABELERFLAG_LOCAL_LABEL_SUPPORT)){
...
int parse_file(Parser* parser, Mc_stream_t* files_stream){
    ...
    for(;
        ...
        const uint8_t instop = get_inst_op(token);
        inst_profile = (InstProfile){.opcode = instop, .op_profile = get_inst_profile(instop)};
        if(inst_profile.opcode == INST_ERROR){
            const Token next_token = get_next_token(parser->tokenizer);
            if((next_token.type == TKN_SPECIAL_SYM) && (token.type == TKN_RAW)){
                if(next_token.value.as_char == ':'){
                    if(add_label(&parser->labeler, token, (Token){.value.as_uint = parser->program->size / 4, .type = TKN_INST_POSITION}, parser->program->size / 4))
                    {
                        REPORT_ERROR(
                            parser,
